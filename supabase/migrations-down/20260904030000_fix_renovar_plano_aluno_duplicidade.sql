-- Reverte 20260904030000_fix_renovar_plano_aluno_duplicidade.sql.
--
-- Restaura o corpo confirmado via pg_get_functiondef() como em execução
-- em staging e produção antes desta migration (idêntico ao arquivo
-- supabase/migrations/20260903030000_fix_renovar_plano_aluno_super_admin.sql)
-- — reintroduz o bug do PED-160: reassinar um aluno (mesmo plano ou
-- outro) no mesmo mês em que já existe mensalidade pendente volta a
-- gerar uma segunda mensalidade cobrindo o mesmo período.
