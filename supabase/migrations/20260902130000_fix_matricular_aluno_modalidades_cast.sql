-- Corrige matricular_aluno(): a coluna alunos.modalidades_selecionadas é
-- uuid[], mas p_modalidades chega como jsonb (o cliente serializa o array
-- de IDs via supabase.rpc()). A atribuição direta `modalidades_selecionadas
-- = p_modalidades` sempre falhava com 42804 ("column is of type uuid[] but
-- expression is of type jsonb"), impedindo qualquer matrícula de aluno em
-- um plano (a função nunca chegava a gravar historico_planos/mensalidades).
-- Extrai os elementos do array jsonb como texto e converte para uuid[].
