-- PED-127: mesmo bug do PED-122/126, agora em renovar_plano_aluno (fluxo de
-- renovação manual de plano). A função só aceitava uma linha literal
-- role='admin' em estudio_membros pro estúdio impersonado, sem o bypass de
-- eh_super_admin() já usado no resto do app. Um super_admin impersonando
-- receberia "Acesso negado: você não é admin deste estúdio." ao confirmar
-- a renovação.
