-- PED-78: a comparação de md5(pg_get_functiondef()) entre staging e
-- produção apontou 19 das 38 functions capturadas em
-- 20260827141042_capture_missing_functions.sql (PED-30) como "divergentes"
-- (hash diferente). Investigação (ver comentário na issue) provou que a
-- divergência é 100% ruído de formatação: os corpos capturados de staging
-- usam CRLF (herdado de uma edição direta via SQL editor em algum ponto
-- antes da PED-30 — daí o comentário "CRLF preservado" no commit
-- fd9078b), enquanto produção nunca foi tocada e mantém os corpos originais
-- da baseline (00000000000000_baseline_current_schema.sql), que usam LF.
--
-- Prova: comparando md5(replace(pg_get_functiondef(oid), E'\r\n', E'\n'))
-- nos dois projetos (staging qjmybxkfjkxttggdjxga x produção
-- tciiepqmnrrcjnqhspvw) em 2026-08-29, as 19 functions batem exatamente
-- após normalizar quebra de linha — nenhuma diferença de lógica. Não há
-- hotfix de produção sendo sobrescrito (a preocupação original da PED-78):
-- produção está simplesmente intacta desde a baseline.
--
-- Esta migration recria as 19 functions com o corpo canônico de produção
-- (LF, sem CRLF), aplicada em ambos os ambientes — elimina o drift
-- definitivamente em vez de só validar que é seguro ignorá-lo, e evita que
-- o CRLF acidental volte a ser capturado como "divergência real" numa
-- futura comparação. Comportamento inalterado (mesma lógica, só a
-- codificação da quebra de linha no prosrc muda).
--
-- As 8 functions fake_* (seed/QA) da PED-30 ficam de fora de propósito:
-- só são referenciadas em supabase/seed-staging/, sem nenhum caller em
-- produção — não há motivo para adicioná-las lá.
