-- App mobile Área do Aluno (docs/superpowers/specs/2026-09-09-app-mobile-area-aluno-design.md):
-- 1) regras de cancelamento/inadimplência configuráveis por estúdio;
-- 2) libera o próprio aluno a chamar agendar_avulso (hoje só admin/professor,
--    via RLS de presencas — precisa virar SECURITY DEFINER com checagem
--    própria, no mesmo padrão já usado em cancelar_agendamento).

alter table public.estudios
  add column if not exists horas_min_cancelamento integer not null default 4,
  add column if not exists dias_tolerancia_inadimplencia integer not null default 5;

comment on column public.estudios.horas_min_cancelamento is
  'Antecedência mínima, em horas, para o aluno cancelar um agendamento próprio.';
comment on column public.estudios.dias_tolerancia_inadimplencia is
  'Dias de atraso (alunos.dias_atraso) a partir dos quais o aluno fica bloqueado de novos agendamentos.';

-- agendar_avulso: SECURITY DEFINER + checagem de propriedade (aluno só agenda
-- a si mesmo; admin/professor/super_admin mantêm o poder de agendar qualquer
-- aluno do estúdio, como hoje) + bloqueio por inadimplência (P0102 — código já
-- esperado pelo frontend em webapp/src/pages/Agenda/hooks/useAgendamento.js,
-- nunca implementado no backend até agora).
...
begin
  ...
  -- AUTORIZAÇÃO — antes só a RLS de presencas controlava isso (admin/professor
  -- apenas). Como a function virou SECURITY DEFINER, a checagem precisa ser
  -- explícita aqui, senão qualquer autenticado poderia agendar qualquer aluno.
  select exists (
  ...
  if not v_pode_gerenciar then
    ...
    if v_auth_id_aluno is distinct from auth.uid() then
  ...
  if not p_ignorar_avisos then
    -- Inadimplência: bloqueia ANTES de checar disponibilidade (falha rápida).
    -- alunos.dias_atraso já é mantido pelo cron existente — só compara contra
    -- o limiar configurável do estúdio.
...
-- cancelar_agendamento: adiciona a checagem de antecedência mínima
-- (horas_min_cancelamento). Mantém 100% do comportamento/assinatura
-- existente — SECURITY DEFINER e checagem de auth.uid() já existiam.
...
begin
  ...
  -- Antecedência mínima: só bloqueia quando dá pra saber o horário da aula
  -- (agenda.horario) — se a aula já não existir mais, deixa o delete seguir
  -- e cair no "nenhum agendamento encontrado" abaixo, sem falso bloqueio.
