...
import { Bell, Cake, Package, CheckCircle2, RotateCcw, AlertCircle, WifiOff } from 'lucide-react';
...
export default function Notificacoes() {
  // FIX: `error` agora é desestruturado — o hook já o expõe de propósito
  // para não confundir "falha ao carregar" com "nenhuma notificação pendente".
  const { ativas, concluidas, loading, error, marcarComoResolvida, desfazerResolvida } = useNotificacoes();
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      {/* FIX: estado de erro tratado explicitamente — antes caía no mesmo
          branch de "Tudo em dia!" quando a query falhava. */}
      {error ? (
        <Surface variant="card" padding="xl" className="text-center border-destructive/30">
          <WifiOff size={40} className="mx-auto text-destructive mb-3" />
          <p className="font-black text-foreground">Não foi possível carregar as notificações.</p>
