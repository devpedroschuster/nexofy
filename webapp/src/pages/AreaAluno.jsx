...
// FIX: monta a data local (ano-mes-dia) sem passar por toISOString(),
// que converte para UTC e pode deslocar o dia em fusos negativos (Brasil).
const paraDataLocalStr = (d) => {
...
const gerarProximosDias = () => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const dataLocal = paraDataLocalStr(d);
    dias.push({
      dataIso: dataLocal,
      diaSemana: i === 0 ? 'Hoje' : NOMES_DIAS[d.getDay()],
      diaMes: dataLocal.split('-').slice(1).reverse().join('/'),
...
export default function AreaAluno() {
  ...
  const [diaAtivo, setDiaAtivo] = useState(proximosDias[0].dataIso);
  ...
  const { data: presencasMes } = useQuery({
    ...
    queryFn: async () => {
      ...
      const primeiroDiaStr = paraDataLocalStr(primeiroDiaMes);
      const ultimoDiaStr = paraDataLocalStr(ultimoDiaMes);
      ...
      const [{ data: presencas, error }, { data: feriados }] = await Promise.all([
        supabase
          ...
          .eq('estudio_id', estudioIdAluno)
          .gte('data_checkin', primeiroDiaStr)
          .lte('data_checkin', ultimoDiaStr), // FIX: limite superior que faltava
        supabase
          .from('feriados')
          .select('data')
          .eq('estudio_id', estudioIdAluno)
  ...
  const { data: aulasDoDia, isLoading: loadingAulas, isError: erroAulas } = useQuery({
    queryKey: ['agenda', diaAtivo, estudioIdAluno],
    enabled: !!aluno?.id && !!estudioIdAluno,
    queryFn: async () => {
      const diaSelecionado = proximosDias.find((d) => d.dataIso === diaAtivo);
      // FIX: guarda contra diaSelecionado indefinido em vez de estourar TypeError
  ...
  const { data: feriadosDaSemana } = useQuery({
    queryKey: ['feriados-semana', estudioIdAluno],
    enabled: !!estudioIdAluno,
    queryFn: async () => {
  ...
  const handleAvatarUpload = async (event) => {
    ...
    if (file.size > AVATAR_MAX_BYTES) {
      showToast.error('Imagem muito grande. O limite é 5MB.');
      event.target.value = '';
      return;
    }
    ...
    try {
      ...
      showToast.error(error.message || 'Erro ao enviar a imagem.');
    } finally {
  ...
  // FIX: número de suporte vem da configuração do estúdio (multi-tenant),
  // não mais hardcoded — cada estúdio direciona para o próprio WhatsApp.
  const openWhatsApp = (msg) => {
    const numero = estudio?.telefone_suporte?.replace(/\D/g, '');
    if (!numero) {
      showToast.error('Número de suporte não configurado para este estúdio.');
      return;
    }
  ...
  if (erroAluno || !aluno) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-[#FDF8F5]">
        <p className="text-gray-600 mb-4">Erro ao carregar dados do aluno.</p>
        <button onClick={doLogout} className="btn btn-primary">Voltar para o Início</button>
      </div>
    );
  }
  ...
  return (
    <div id="page-dashboard">
      <aside className="sidebar">
        ...
        <button className={`nav-item ${abaAtiva === 'schedule' ? 'active' : ''}`} onClick={() => setAbaAtiva('schedule')}>
          <span className="nav-icon">📅</span> Agendar Aulas
        </button>
        <button className={`nav-item ${abaAtiva === 'profile' ? 'active' : ''}`} onClick={() => setAbaAtiva('profile')}>
          <span className="nav-icon">👤</span> Meu Perfil
        </button>
        <button className={`nav-item ${abaAtiva === 'payments' ? 'active' : ''}`} onClick={() => setAbaAtiva('payments')}>
          <span className="nav-icon">💳</span> Mensalidades
        </button>
        {isProfessor && (
          <button
            className="nav-item"
        ...
        <div className="sidebar-footer">
          <button className="nav-item" onClick={doLogout} style={{ color: 'var(--err)' }}>
      ...
      <div className="main-content">
        {abaAtiva === 'schedule' && (
          <div>
            <div className="main-header">
              <div>
                <div className="main-header-title">Agendar Aulas</div>
                <div className="main-header-sub">Escolha o dia e reserve sua vaga</div>
              </div>
            </div>
            <div className="main-body">
                ...
                <div className="mb-8 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {regrasPlano.map((regra, idx) => {
                      const usosDaRegra = consumoPorArea.get(regra.modalidade) || 0;
                          ...
                          <div className="flex justify-between items-end mb-2">
                            <div>
                              ...
                              <p className="text-xs text-gray-400 font-medium mt-0.5">
                                Usado: <span className="text-primary font-bold text-sm">{usosDaRegra}</span> /{' '}
                                {limit === 999 ? 'Ilimitado' : limit}
                              </p>
                            </div>
                          ...
                          {limit !== 999 && (
                            <div className="bg-gray-200 h-1.5 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{ width: `${pct}%`, backgroundColor: pct >= 100 ? 'var(--err)' : 'var(--pri)' }}
                              ></div>
                            </div>
                          )}
                        </div>
              ...
              <div className="day-tabs">
                {proximosDias.map((d) => (
                  <button
                    key={d.dataIso}
                    className={`day-tab ${diaAtivo === d.dataIso ? 'active' : ''}`}
                    onClick={() => setDiaAtivo(d.dataIso)}
                    style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', padding: '8px 16px' }}
                  >
              ...
              <div id="class-list">
                {loadingAulas ? (
                  ...
                  <div className="text-center py-14 bg-white rounded-3xl border border-gray-100 shadow-sm">
                    ...
                    <p className="text-gray-500 text-sm">Tente novamente em instantes.</p>
                  </div>
                ) : !aluno.planos ? (
                  <div className="text-center py-14 bg-white rounded-3xl border border-gray-100 shadow-sm">
                  ...
                  aulasPermitidas.map((aula) => {
                    ...
                    const usosDestaAula = regraDestaAula ? consumoPorArea.get(aulaArea) || 0 : 0;
                    ...
                    return (
                      <div key={aula.id} className={`class-card anim-fade-up ${jaAgendado ? 'booked' : ''}`}>
                        <div className="class-time-block">
                          ...
                          <div className={`class-space ${aulaArea === 'Dança' ? 'danca' : 'funcional'}`}>{aulaArea}</div>
                        </div>
                          ...
                          <div className="capacity-bar-row">
                            ...
                            <div className="capacity-bar-track">
                              <div
                                ...
                                style={{
                                  width: `${((aula.vagas_ocupadas || 0) / aula.capacidade) * 100}%`,
                                  background: jaAgendado ? 'var(--ok)' : lotado ? 'var(--err)' : 'var(--pri)',
                                }}
                              ></div>
                            </div>
                          </div>
            ...
            <div className="main-body">
              <div className="profile-top">
                ...
                <div className="profile-plan-card card">
                  ...
                  <div className="profile-plan-desc">
                    {aluno.planos ? `Valor: ${formatarMoeda(aluno.planos.preco)}` : 'Você ainda não possui um plano vinculado.'}
                  </div>
                </div>
              </div>
              <div className="card" style={{ marginBottom: '24px' }}>
                <div
                  style={{ fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '2px', color: 'var(--muted)', marginBottom: '24px' }}
                >
                  Dados Pessoais
                </div>
                <div className="data-grid">
                  <div className="data-field">
                    ...
                    <div className="data-value opacity-60 cursor-not-allowed">{aluno.email}</div>
                  </div>
                  <div className="data-field">
                    <label>Telefone</label>
                    {modoEdicao ? (
                      <input
                        className="inp py-2 mt-1"
                        value={formEdit.telefone}
                        onChange={(e) => setFormEdit({ ...formEdit, telefone: e.target.value })}
                        placeholder="(00) 00000-0000"
                      />
                    ) : (
                      <div className="data-value">{aluno.telefone || 'Não informado'}</div>
                    )}
                  </div>
                  <div className="data-field">
                    <label>CPF</label>
                    {modoEdicao ? (
                      ...
                      <div className="data-value">{aluno.cpf || 'Não informado'}</div>
                    )}
                  </div>
                  <div className="data-field">
                    ...
                    {modoEdicao ? (
                      <input
                        ...
                        onChange={(e) => setFormEdit({ ...formEdit, data_nascimento: e.target.value })}
                      />
                    ) : (
                      <div className="data-value">{formatarData(aluno.data_nascimento) || 'Não informado'}</div>
                    )}
                  </div>
                </div>
              </div>
              <div className="wa-btn-row">
                <button
                  ...
                  onClick={() => openWhatsApp(`Olá, ${nomeEstudio}! Sou o aluno(a) ${aluno.nome_completo} e preciso de ajuda.`)}
        ...
        {abaAtiva === 'payments' && (
          <div>
            <div className="main-header">
              <div>
                <div className="main-header-title">Mensalidades</div>
                <div className="main-header-sub">Histórico e status dos seus pagamentos</div>
              </div>
            </div>
            <div className="main-body">
              <div className="card">
