...
const MODALIDADE_VAZIA = {
  ...
  taxa_professor: 50, taxa_espaco: 50, taxa_direcao: 0
};
  ...
  async function handleSalvarEdicao(e) {
    ...
    try {
      ...
      if (modPerfil && modPerfil.id === modalidadeEmEdicao.id) {
        setModPerfil({ ...modPerfil, ...payload });
      }
      fetchDados();
    } catch (err) {
    ...
    <div className="p-8 space-y-8 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loadingList ? (
          ...
          <Surface variant="card" padding="lg" className="col-span-full text-center text-sm text-muted-foreground">
                    ...
                    {dadosPerfil.horarios.length === 0 ? (
                      ...
                      <ul className="flex flex-col gap-2">
                        {dadosPerfil.horarios.map((h, i) => (
                          <li key={`${h.dia_semana}-${h.horario}-${i}`} className="bg-purple-soft text-purple border border-purple/20 px-3 py-3 rounded-xl text-sm font-bold flex items-center gap-3">
