...
const SIZES = { sm: 'max-w-md', md: 'max-w-xl', lg: 'max-w-2xl', xl: 'max-w-4xl', full: 'max-w-[95vw]' };
const SIZES_LEGACY = { sm: 'max-w-sm', md: 'max-w-md', lg: 'max-w-lg', xl: 'max-w-xl', '2xl': 'max-w-2xl' };

function Modal({
  aberto, fechar, title, description, size = 'md',
  closeOnOverlay = true, hideClose = false, className, children,
  isOpen, onClose, titulo, tamanho, fecharAoClicarFora, footer,
}) {
  const _aberto = aberto ?? isOpen ?? false;
  const _fechar = fechar ?? onClose ?? (() => {});
  const _title = title ?? titulo;
  const _closeOnOverlay = fecharAoClicarFora !== undefined ? fecharAoClicarFora : closeOnOverlay;
  ...
  return createPortal(
    <div role="dialog" aria-modal="true" aria-label={_title}
      className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      <div onClick={_closeOnOverlay ? _fechar : undefined}
      ...
      <div onClick={(e) => e.stopPropagation()}
        className={cn(
          ...
          _sizeClass, className
        )}>
        {(_title || !hideClose) && (
          <header className="flex items-start justify-between gap-4 px-6 pt-6 pb-4 border-b border-border shrink-0">
            <div className="min-w-0">
              {_title && <h2 className="text-lg font-black tracking-tight text-foreground truncate">{_title}</h2>}
              {description && <p className="mt-1 text-sm text-muted-foreground">{description}</p>}
            </div>
            {!hideClose && (
              <button onClick={_fechar} aria-label="Fechar"
...
Modal.Footer = function ModalFooter({ className, children }) {
  return (
    <div className={cn('mt-6 -mx-6 -mb-5 flex flex-wrap items-center justify-end gap-2 border-t border-border bg-muted/40 px-6 py-4', className)}>
      {children}
    </div>
  );
};
...
export function ModalConfirmacao({
  ...
  const bgIcone = {
    danger: 'bg-destructive-soft', warning: 'bg-warning-soft',
    success: 'bg-success-soft', info: 'bg-info-soft',
  };
  const btnVariant = {
    danger: 'destructive', success: 'success', info: 'info', warning: 'ghost',
  };
  ...
  return (
    <Modal aberto={_aberto} fechar={_fechar} title="" size="sm" hideClose>
      <div className="text-center space-y-4 py-2">
        <div className={cn('w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6', bgIcone[tipo])}>
          {icones[tipo]}
        </div>
